...
static inline esp_err_t
esp_websocket_client_stop(esp_websocket_client_handle_t client) {
...
esp_websocket_client_destroy(esp_websocket_client_handle_t client) {
  ESP_LOGI("[esp_websocket_client_stub]",
           "esp_websocket_client_destroy called");
  return ESP_OK;
}

static inline int
esp_websocket_client_send_text(esp_websocket_client_handle_t client,
                               const char *data, int len, TickType_t timeout) {
  ESP_LOGI("[esp_websocket_client_stub]",
           "esp_websocket_client_send_text called");
  return 1;
}

static inline bool
esp_websocket_client_is_connected(esp_websocket_client_handle_t client) {
  ESP_LOGI("[esp_websocket_client_stub]",
           "esp_websocket_client_is_connected called");
  return true;
}

static inline esp_err_t esp_websocket_register_events(
    esp_websocket_client_handle_t client, esp_websocket_event_id_t event,
    esp_event_handler_t event_handler, void *event_handler_arg) {
  ESP_LOGI("[esp_websocket_client_stub]",
           "esp_websocket_register_events called");
  return ESP_OK;
}
