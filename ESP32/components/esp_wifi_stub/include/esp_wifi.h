...
static inline esp_err_t esp_wifi_connect(void) {
  ESP_LOGI("[esp_wifi_stub]", "esp_wifi_connect called");
  return ESP_OK;
}

static inline esp_err_t esp_wifi_disconnect(void) {
  ESP_LOGI("[esp_wifi_stub]", "esp_wifi_disconnect called");
  return ESP_OK;
}

static inline esp_err_t esp_wifi_scan_start(const wifi_scan_config_t *config,
                                            bool block) {
...
static inline esp_err_t esp_wifi_sta_get_ap_info(wifi_ap_record_t *ap_info) {
  ESP_LOGI("[esp_wifi_stub]", "esp_wifi_sta_get_ap_info called");
  return ESP_OK;
}
