...
void change_esp_id(char *name) {
  if (strncmp(name, "bms_", 4) != 0) {
    ESP_LOGE(
        "utils",
        "New name not formatted correctly: \"%s\", should begin with \"bms_\"",
        name);
    return;
  }
  char *id_str = &name[4];
...
void initialise_spiffs() {
  esp_vfs_spiffs_conf_t config_static = {.base_path = "/static",
...
void send_fake_request() {
  if (!connected_to_WiFi) {
    ...
    if (nvs_get_str(nvs, "SSID", ssid, &len) != ESP_OK)
      ESP_LOGW("utils",
               "Could not read Wi-Fi router SSID from NVS, using default");
    len = sizeof(password);
    if (nvs_get_str(nvs, "PASSWORD", password, &len) != ESP_OK)
      ESP_LOGW("utils",
               "Could not read Wi-Fi router password from NVS, using default");
    if (nvs_get_u8(nvs, "AUTO_CONNECT", &auto_connect) != ESP_OK)
      ESP_LOGW(
          "utils",
          "Could not read Wi-Fi auto-connect setting from NVS, using default");
...
esp_err_t fake_login_http_event_handler(esp_http_client_event_t *evt) {
  ...
  case HTTP_EVENT_ON_DATA:
    if (!esp_http_client_is_chunked_response(evt->client)) {
      ...
      int copy_len = evt->data_len <= user_data->buffer_len - 1
                         ? evt->data_len
                         : user_data->buffer_len - 1;
...
char *send_fake_login_post_request() {
  ...
  snprintf(post_data, sizeof(post_data),
           "{\"email\":\"%s\",\"password\":\"%s\"}", encoded_email,
           encoded_password);
  ...
  if (err == ESP_OK) {
    if (VERBOSE)
      ESP_LOGI("MESH", "HTTP POST Status = %d, Response = %s",
               esp_http_client_get_status_code(client), response.buffer);
...
void convert_uint_to_n_bytes(unsigned int input, uint8_t *output,
                             size_t n_bytes, bool little_endian) {
...
float calculate_symbol_length(uint8_t spreading_factor, uint8_t bandwidth) {
  float frequencies[] = {7.8,  10.4, 15.6,  20.8,  31.25,
...
int calculate_transmission_delay(uint8_t spreading_factor, uint8_t bandwidth,
                                 uint8_t n_preamble_symbols,
                                 uint16_t payload_length, uint8_t coding_rate,
                                 bool header, bool low_data_rate_optimisation) {
  float t_sym = calculate_symbol_length(spreading_factor, bandwidth);
  if (VERBOSE)
    ESP_LOGI("LoRa", "symbol time: %f ms", t_sym);

  float n_payload_symbols =
      ceil((8 * payload_length - 4 * spreading_factor + 28 + 16 -
            (header ? 20 : 0)) /
           (spreading_factor - (low_data_rate_optimisation ? 2 : 0)) / 4);
  ...
  if (VERBOSE)
    ESP_LOGI("LoRa", "N payload symbols: %f", n_payload_symbols);

  float t_air = (4.25 + (float)n_preamble_symbols + n_payload_symbols) * t_sym;
  if (VERBOSE)
    ESP_LOGI("LoRa", "time on air: %f ms", t_air);

  float n_payloads_per_hour = 36e3 / t_air;
  if (VERBOSE)
    ESP_LOGI("LoRa", "number of payloads per hour: %f", n_payloads_per_hour);

  float time_delay_seconds = pow(60, 2) / n_payloads_per_hour;
  if (VERBOSE)
    ESP_LOGI("LoRa", "delay between messages: %f s", time_delay_seconds);
