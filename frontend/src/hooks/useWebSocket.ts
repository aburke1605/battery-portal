...
export const createMessage = (summary: string, data: any, esp_id?: number) => ({
	type: "request",
	content: {
		summary,
    ...
    */
	try {
		const res = await axios.get(
			`${apiConfig.DB_DATA_API}?esp_id=${battery.esp_id}`,
		);
		...
		if (battery.nodes && battery.nodes.length > 0) {
			const mergedNodes = await Promise.all(
				battery.nodes.map(appendBatteryData),
			);
...
// recursive helper
const extractSingleBattery = (
	info: BatteryData[] | BatteryInfoData[],
	esp_id: number,
): any => {
	/*
