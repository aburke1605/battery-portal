  ...
  return (
    <div key={battery.esp_id} className="bg-white rounded-lg shadow p-4">
      <div className="flex justify-between items-start mb-3">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Last Updated:{" "}
            {format(new Date(battery.last_updated_time), "yyyy-MM-dd HH:mm:ss")}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span
            className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(battery.live_websocket)}`}
          >
      ...
      <div className="space-y-2">
        ...
        <div
          className={`space-y-2 ${battery.nodes && battery.nodes.length > 3 ? "max-h-[200px] overflow-y-auto pr-1 custom-scrollbar" : ""}`}
        >
          {battery.nodes?.map((childBattery) => (
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(childBattery.live_websocket)}`}
                >
