...
export default function BatteryPage({
    ...
    if (updates.new_esp_id && updates.new_esp_id !== esp_id) {
      ...
      !isFromESP32
        ? window.location.replace(
            `/#/battery-detail?esp_id=${updates.new_esp_id}`,
          )
        : (window.location.href = `/esp32?esp_id=${updates.new_esp_id}#/`);
    }
  };

  const sendWiFiConnect = (
    ssid: string,
    password: string,
    auto_connect: boolean,
  ) =>
    sendMessage(
      createMessage("connect-wifi", { ssid, password, auto_connect }, esp_id),
    );

  const sendUnseal = () => sendMessage(createMessage("unseal-bms", {}, esp_id));
  ...
  return (
    <div>
      {!isFromESP32 ? (
        <PageMeta title="Battery Dashboard" description="Battery Detail Page" />
      ) : null}
      {!isFromESP32 ? <PageBreadcrumb pageTitle="Battery Detail" /> : null}
