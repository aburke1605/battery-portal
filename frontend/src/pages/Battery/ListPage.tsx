  ...
  return (
    <div>
      {!user?.subscribed ? (
        <>
          <p>Not subscribed</p>
          <a className="hover:text-blue-400" href="/#/subscription">
            {`>`} Subscribe now!
          </a>
        </>
      ) : (
        <main className="container mx-auto px-4 py-8">
          ...
          <div
            className={
              viewMode === "map"
                ? "bg-white rounded-lg shadow overflow-hidden w-full"
                : "hidden"
            }
          >
              ...
              {selectedESPID &&
                (() => {
                  const selectedBattery = batteries.find(
                    (b) => b.esp_id === selectedESPID,
                  );
                  if (!selectedBattery) return null;
                  return (
                    <div className="p-3 min-w-[200px]">
                      <h3 className="font-semibold text-lg">
                        {selectedBattery.esp_id}
                      </h3>
                      <p className="text-sm text-gray-600 mt-1">
                        Status:{" "}
                        {selectedBattery.live_websocket ? "online" : "offline"}
                      </p>
            ...
            <div className="mt-8 flex flex-col items-center gap-4">
              <div className="flex items-center justify-between w-full max-w-2xl">
                <div className="text-sm text-gray-600">
                  Showing {startIndex + 1} to {endIndex} of {batteries.length}{" "}
                  entries
                </div>
