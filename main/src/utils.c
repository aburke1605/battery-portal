...
char *send_fake_login_post_request() {
    ...
    if (err == ESP_OK) {
        ESP_LOGI("HTTP_CLIENT", "HTTP POST Status = %d, Response = %s", esp_http_client_get_status_code(client), response.buffer);
...
int calculate_transmission_delay(uint8_t spreading_factor, uint8_t bandwidth, uint8_t n_preamble_symbols, uint16_t payload_length, uint8_t coding_rate, bool header, bool low_data_rate_optimisation) {
    ...
    if (VERBOSE) ESP_LOGI("LoRa", "symbol time: %f ms\n", t_sym);
    ...
    if (VERBOSE) ESP_LOGI("LoRa", "N payload symbols: %f\n", n_payload_symbols);

    float t_air = (4.25 + (float)n_preamble_symbols + n_payload_symbols) * t_sym;
    if (VERBOSE) ESP_LOGI("LoRa", "time on air: %f ms\n", t_air);

    float n_payloads_per_hour = 36e3 / t_air;
    if (VERBOSE) ESP_LOGI("LoRa", "number of payloads per hour: %f\n", n_payloads_per_hour);

    float time_delay_seconds = pow(60, 2) / n_payloads_per_hour;
    if (VERBOSE) ESP_LOGI("LoRa", "delay between messages: %f s\n", time_delay_seconds);
